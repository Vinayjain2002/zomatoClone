...
const getCollection = async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    try {
      // getting only the collections field from the whole data
...
const getLocalities = async (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    try {
      // need to extract only locality and cords from the data
...
const getRestaurant = async (req, res) => {
  ...
  console.log(query);
  try {
    // finding the restaurants based on the score and sorting the data based on the score as well
    let result = await Restaurant.find(
      {
        "location.city_id": parseInt(city_id),
        // searching the data based on the search index mentioned as the text
        $text: {
          $search: query,
